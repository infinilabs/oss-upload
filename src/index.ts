 ...
 */
async function repackFiles(folder: string, filePattern: string, version: string): Promise<string[]> {
  ...
  const filesToRepack: string[] = glob.sync(`${folder}/${filePattern}`, { nodir: true });
    ...
    for (const oldFilePath of filesToRepack) {
        ...
        for (const pattern in fileMap) {
            ...
            if (regex.test(baseName)) {
                ...
                const originalPrefix = fileNameWithoutExt.replace(/[-_]*[a-zA-Z0-9]+$/, '');

                // Construct the new base name dynamically
                ...
                try {
                    ...
                    const zipFilePath = path.join(path.dirname(oldFilePath), `${newBaseName}.zip`);
...
async function run(): Promise<void> {
  try {
      ...
      const results = await Promise.allSettled(
          Array.from({ length: maxConcurrency }, async (_, index): Promise<number> => {
              return new Promise<number>((resolve) => {
                  const proc = async (): Promise<void> => {
                      ...
                      try {
                          const ossFilePath = path.posix.join(remoteDir, path.relative(folder, file));
