...
const path = tslib_1.__importStar(require("path")); // 使用 path.posix
async function run() {
    try {
        ...
        const files = glob_1.glob.sync(`${folder}/${FILE_PATTERN}`, {
            nodir: true
        });
        // 使用 Promise.allSettled 避免一个文件上传失败导致整个进程失败
        const results = await Promise.allSettled(Array.from({ length: maxConcurrency }, async (_, index) => {
